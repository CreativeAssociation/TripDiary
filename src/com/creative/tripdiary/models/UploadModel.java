 ...
 */
package com.creative.tripdiary.models;
...
public class UploadModel extends TransferModel {
    ...
    public UploadModel(Context context, Uri uri, TransferManager manager, String memberId) {
        super(context, uri, manager, memberId);
    ...
    public void upload() {
        ...
        if(mFile != null) {
            try {
            	//Util.getPrefix(getContext())
                mUpload = getTransferManager().upload(
                        Constants.BUCKET_NAME,
                        	getMemberId()+"/"+super.getFileName() + "."
                            + mExtension,
                        mFile);
                mUpload.addProgressListener(mListener);
            } catch(Exception e) {
